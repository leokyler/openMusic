...
- **FR-003a**: System MUST 格式化复制的提示词时，按固定顺序包含所有非空字段：
  1. lyrics（如果不为 null）
  2. style（如果不为 null）
  3. vocal（如果不为 null）
  4. instrumental（如果不为 null）
     每个字段使用英文字段名称后跟冒号和换行符标记，字段之间用空行分隔
...
- **SC-001**: 用户点击复制按钮后，客户端剪贴板操作（Clipboard API 调用）在 100ms 内完成，异步追踪 API 调用不阻塞此操作
