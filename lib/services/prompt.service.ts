...
export class PromptService {
  ...
  async createPrompt(data: CreatePromptDto): Promise<Prompt> {
    ...
    const prompt = await prisma.prompt.create({
      data: {
        lyrics: data.lyrics,
        style: data.style,
        vocal: data.vocal as any,
        instrumental: data.instrumental as any,
        qualityScore: qualityResult.score,
        qualityWarnings: qualityResult.warnings as any,
      },
    });
  ...
  async listPrompts(options: PromptListOptions = {}): Promise<PaginatedResult<Prompt>> {
    ...
    const promptsWithCount = items.map((item) => ({
      ...item,
      outputCount: (item as any)._count.outputs,
    }));
